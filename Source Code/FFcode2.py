...
def b_spline_basis(U, degree=4, num_knots=12):
...
def i_spline_basis(U, degree=4, num_knots=12):
...
def g(U, b_params, degree=4, num_knots=12):
    U = np.atleast_1d(U)  
...
def log_likelihood(params, U, Baseline, D_obs, Delta):
    ...
    return -likelihood + penalty_weight * constraint_violation + 10*abs(a3 + 3*a4) + abs(1/a3) + abs(1/a4) 
...
def estimate_parameters(U, Baseline, D_obs, Delta, num_initializations=20):
    ...
    initial_params[:,0] = np.random.uniform(1, 20, num_initializations)  
    initial_params[:, 1] = np.random.uniform(-0.5, 0.5, num_initializations)  
    random_numbers = np.random.rand(num_initializations, 3)
    initial_params[:,2:5] = random_numbers / np.sum(random_numbers, axis=1, keepdims=True)  
    initial_params[:,5] = np.random.uniform(3, 10, num_initializations)  
    initial_params[:,6] = np.random.uniform(400, 500, num_initializations)  
    random_numbers = np.random.rand(num_initializations, 7)
    initial_params[:, 7:14] = random_numbers / np.sum(random_numbers,axis=1, keepdims=True)  
...
def load_data(csv_file, split_ratio=0.7):
    ...
    Baseline_raw = data.iloc[:, 17].values#7
    D_obs_raw = data.iloc[:, 16].values#6
